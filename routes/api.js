...
router.post('/create-job', async (req, res) => {
    ...
    try{
        ...
        const result = await jobs.createJob(req.session.email, job.title, job.jobtype, job.fieldtype, job.location, job.description, job.experience, job.salaryrange, job.vacancy, job.status);
        return res.status(200).json(result);
    }
    catch(error)
    {
...
router.post('/create-feedback', async (req, res) => {
    ...
    try{
        const applicationList = await applications.getByCompany(feedback.companyId);
        if(await applicationList.find(x=>x.jobseekerId == feedback.userId))
        {
            const result = await feedbacks.createFeedback(feedback.comments, feedback.rating, feedback.userId, feedback.companyId);
            return res.status(200).json(result);
        }
        else{
            return res.status(400).json({ error : "User have no rights to give feedback for this company" });
        }
    }
    catch(error)
    {
...
//get feedbacks by companyId
router.get('/get-feedbacks/:id', async (req,res) => {
    const companyId = req.params.id;
    try{
        const feedbackList = await feedbacks.get(companyId)
